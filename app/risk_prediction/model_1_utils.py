...
def predict_cervical_cancer_risk(data) -> Tuple:
    ...
    try:
        # load the model
        model = jl.load('risk_prediction/model_1.pk1')
        ...
        if prediction[0] == 0:
            ...
            text_prediction = '0' # ai_api.generate_response(prediction_str)
            return text_prediction
        else:
            prediction_str = 'Please generate only a text of 3 rows about the following prediction: ' \
                             'The predicted risk of cervical cancer is high.'
            text_prediction = '1'  # ai_api.generate_response(prediction_str)
            return text_prediction
